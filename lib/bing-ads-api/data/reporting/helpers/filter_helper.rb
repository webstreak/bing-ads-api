    ...
    module FilterHelper
        ...
        def solve_filter_value(filter_criteria, filter_value)
            ...
            if filter_value.is_a?(String)
                ...
                if filter_value.first.is_a?(Numeric)
                    filter_value = filter_value.map { |f| {"ins0:int" => f } }
                elsif filter_value.first.is_a?(String)
                    filter_value = filter_value.map { |f| {"ins0:string" => f } }
                end
                return filter_value
            end
            return nil
